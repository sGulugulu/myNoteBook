...
L1 和 Shared Memory 虽然共享同一物理 SRAM，但 L1 需要 tag lookup 和缓存控制逻辑，而 Shared Memory 是无缓存、直接寻址的程序员管理存储，所以访问延迟更低且更稳定

| 特性维度       | **L1缓存**                                                                                                                                | **共享内存**                                                 |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| **本质**     | **硬件自动管理的缓存**                                                                                                                           | **软件显式管理的可编程内存**                                         |
| **管理方式**   | 完全由硬件控制。硬件决定缓存什么数据、何时替换。对程序员**透明**。                                                                                                     | 完全由程序员通过代码控制。需要手动声明、加载数据、控制访问。                           |
| **访问方式**   | **隐式访问**。内核代码读写全局/常量内存时，自动经由L1缓存。ld.global , 地址 → tag lookup → 命中判断  <br>→ (可能) miss → L2 / HBM  <br>→ coherence / replacement , 多了很多步骤 | **显式访问**。内核代码必须使用`__shared__`变量或API来读写。smem[idx]进行访问     |
| **数据生命周期** | 由硬件替换算法（如LRU）决定，与数据是否被使用有关。                                                                                                             | 与**线程块**绑定。线程块开始时分配，块内所有线程可见，块结束时释放。                     |
| **性能关键**   | **局部性**。应优化程序的空间/时间局部性以提高命中率。不保证命中,命中也要走tag pipeline                                                                                    | **访问模式**。必须精心设计以避免**Bank冲突**和保证**合并访问**。不存在miss,由程序员完全控制 |
...
# cp.async.cg vs cp.async.ca辨析

是cp.async的两个变体

| 指令   | 含义      | 是否使用 L1 |
| --------- | --------- |----- |
| cp.async.cg | cache global | ❌ 绕过 L1 |
| cp.async.ca | cache all    | ✔ 使用 L1 |

.cg = 直接从L2 ->SMEM(绕开L1) 
	global -> L2 -> shared memory
	特点: 
		不污染L1cache,延迟更稳定,更可预测
.ca = 走L1 -> SMEM(允许命中L1)
	Global → L2 → L1 → Shared Memory
	特点：如果数据在 L1 → 更快 , 否则还是要去 L2 ,会占用 L1 资源

只有在数据可能会复用的时候会用cp.async.ca, 可以利用L1cache ,减少L2带宽压力
.cg更常见,在高性能GEMM中几乎总是用.cg, 因为
	tile(k) → 用一次 → 丢弃
	tile(k+1) → 新数据
没有重用, L1毫无意义

反而会 污染L1, 干扰别的global load
